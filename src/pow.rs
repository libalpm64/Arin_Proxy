...
impl PowVerifierPool {
    pub fn new(num_threads: usize, pin_threads: bool) -> Arc<Self> {
        ...
        for i in 0..num_threads.max(1) {
            ...
            std::thread::spawn(move || {
                // thread affinity pinning removed to drop windows-sys dependency
                loop {
                    match rx_cl.recv() {
                        Ok(job) => {
    ...
    pub fn submit(&self, nonce: String, challenge_secret: String, difficulty_bits: usize) -> Receiver<bool> {
        let (tx, rx) = bounded(1);
